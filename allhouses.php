...
<div id="blurred">
    ...
    <div style="padding-left: 20px;padding-right: 20px">
        ...
        $total_pages = strcasecmp(trim($username), "admin") == 0 || strcasecmp(trim($username), "administrator") == 0 ? $mysqli->query("SELECT * FROM houses")->num_rows : $mysqli->query("SELECT * FROM houses WHERE state <> 'sold'")->num_rows;
        ...
        if ($stmt = strcasecmp(trim($username), "admin") == 0 || strcasecmp(trim($username), "administrator") == 0 ? $mysqli->prepare("SELECT * FROM houses ORDER BY building LIMIT ?,?") : $prepare = $mysqli->prepare("SELECT * FROM houses WHERE state <> 'sold' ORDER BY building LIMIT ?,?")) {
            ...
            <div class="footer">
                <div class="box" style="margin: auto;background: none;border: none;width: fit-content">
                    <ul class="pagination" style="margin-bottom: 2px">
                        <?php if ($page > 1): ?>
                            <li class="prev"><a href="allhouses.php?page=<?php echo $page - 1 ?>" class="">Prev</a></li>
                        <?php endif; ?>
